...
if (is_numeric($input->g['ban']) && $_SESSION['logged'] == "yes") {
	if ($user_group['canbanmembers'] == "yes") {
		if ($input->g['ban'] != $user_info['id']) {
			$userdet = $db->fetchRow("SELECT forum_role FROM members WHERE id=" . $db->real_escape_string($input->g['ban']));
			...
			if ($userdet['forum_role'] != 1) {
				...
				$db->update("members", $data, "id=" . $db->real_escape_string($input->g['ban']));
			}
		}
	}
...
if (is_numeric($input->g['suspend']) && $_SESSION['logged'] == "yes") {
	if ($user_group['cansuspendmember'] == "yes") {
		if ($input->g['suspend'] != $user_info['id']) {
			$userdet = $db->fetchOne("SELECT forum_role, status FROM members WHERE id=" . $db->real_escape_string($input->g['suspend']));
			...
			if ($userdet['forum_role'] != 1) {
				...
				$db->update("members", $data, "id=" . $db->real_escape_string($input->g['suspend']));
			}
		}
	}
...
if (is_numeric($input->g['movetopic'])) {
	...
	$topic_id = $db->real_escape_string($input->gc['movetopic']);
	...
	if ($check_topic !== false) {
		...
		if ($input->p['newcategory']) {
			...
			$newcategory = $db->real_escape_string($input->pc['newcategory']);
			$total_posts = $db->fetchOne("SELECT COUNT(*) AS NUM FROM forum_posts WHERE topic_rel=" . $frm_topic['id']);
			$check_board = verify_board($newcategory);
